...
class Workflow:
  def __init__(self, query):
    ...
    self.agentic_workflow.add_edge(START, "check_cache")
    ...
    self.agentic_workflow.add_edge("web_search", "final")
    self.agentic_workflow.add_edge("rag_pipeline", "final")
  ...
  async def analyzer(self, state):
    ...
    prompt = f""" You are an expert routing analyzer. You evaluate user queries and determine the  required data sources.
      Topics currently in internal database: {"\n".join(topics) if topics else "None"}

      1. STOP CONDITION: Set stop_now = True ONLY if the query is a generic greeting (e.g., "hello") or absolute gibberish. Provide a polite stop_reply. Do NOT stop for technical or factual questions.
      
      2. ROUTING LOGIC:
        - If the query directly relates to the 'Topics currently in internal database', generate a 'rag_query'.
        - If the query requires general internet knowledge, coding facts (like React), or recent news, generate a 'web_query'.
  ...
  async def critique(self, state):
    ...
    system_prompt = """
      ...
      1. loop: True if the context is not good or is changes to the query can get better results, otherwise False
  ...
  async def draft_final(self, state):
    ...
    if state.get("reranked_rag_context"):
      ...
      context_parts.extend(rag_text)
    if state.get("web_context"): 
      context_parts.append(state["web_context"])
